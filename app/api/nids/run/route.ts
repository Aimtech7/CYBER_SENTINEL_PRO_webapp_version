...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const r = await fetch(url + '/nids/run', { method: 'POST', headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify(b) })
    const j = await r.json()
    return NextResponse.json(j)
  } catch (e) {
    return NextResponse.json({ error: 'nids_failed' }, { status: 500 })
  }
}
